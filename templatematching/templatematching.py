...
def TemplateMatching(src, temp, stepsize): # src: source image, temp: template image, stepsize: the step size for sliding the template
    ...
    for i in np.arange(0, src.shape[0] - temp.shape[0], stepsize):
        for j in np.arange(0, src.shape[1] - temp.shape[1], stepsize):
            ...
            # ------------------ Put your code below ------------------
            mean_source_window = np.mean(src[i:i+temp.shape[0],j:j+temp.shape[1]])
            var_source_window = np.var(src[i:i+temp.shape[0],j:j+temp.shape[1]])
            # Calculate normalized correlation coefficient (NCC) between source and template
            # ------------------ Put your code below ------------------
            
            N = (src[i:i+temp.shape[0],j:j+temp.shape[1]]-mean_source_window)*(temp-mean_temp)
            M = sum(sum(N[i]) for i in range(len(N)))
            corr = (1/float((temp.shape[0])*(temp.shape[1]))) * M / ((var_temp)*(var_source_window))
